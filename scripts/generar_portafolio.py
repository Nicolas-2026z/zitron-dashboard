...
function renderSvcList(){var po={Alta:0,Media:1,Baja:2,"":3};var tasks=SVC_TASKS.filter(function(t){return t.section===SVC_SECTION;}).sort(function(a,b){if(a.completed!==b.completed)return a.completed?1:-1;return(po[a.prioridad]||3)-(po[b.prioridad]||3);});if(!tasks.length){document.getElementById("svc-tasklist").innerHTML='<p style="font-size:12px;color:#6B7280;padding:8px 0">Sin pedidos.</p>';return;}document.getElementById("svc-tasklist").innerHTML=tasks.map(function(t){var over=!t.completed&&t.due&&new Date(t.due+"T00:00:00")<SVC_TODAY;var dt=t.due?new Date(t.due+"T00:00:00").toLocaleDateString("es-CL",{day:"2-digit",month:"short"}):"";var pill=t.completed?'<span class="stpill p-baja">✓ Completado</span>':(t.prioridad?'<span class="stpill p-'+t.prioridad.toLowerCase()+'">'+t.prioridad+"</span>":'');var otBadge=t.n_ot?('<span class="ot-badge">'+t.n_ot_completadas+"/"+t.n_ot+" OT</span>"):'';var nameHtml=t.url?('<a href="'+t.url+'" target="_blank">'+t.name+'</a>'):t.name;return '<div class="strow"><span class="stname">'+nameHtml+otBadge+'</span>'+pill+'<span class="stassign">'+t.assignee+'</span><span class="stdate'+(over?" ov":"")+'">'+dt+(over?" ⚠":"")+"</span></div>";}).join("");}
